...
const plans = [
  { tag: 'FREE', title: 'PRETREINO Free', price: 'R$ 0', period: 'para começar', text: 'Experimente o ecossistema PRETREINO com treino, alimentação, evolução e IA com limites inteligentes.', action: 'Começar grátis' },
  { tag: 'MAIS ESCOLHIDO', title: 'PRETREINO Pro', price: 'R$ 39,90', period: '/mês', annual: 'ou R$ 299,90/ano', text: 'Mais profundidade, personalização e inteligência para continuar a sua evolução.', action: 'Conhecer o Pro', highlight: true },
  { tag: 'EXPERIÊNCIA COMPLETA', title: 'PRETREINO Premium', price: 'R$ 59,90', period: '/mês', annual: 'ou R$ 449,90/ano', text: 'A experiência mais completa, com IA avançada, personalização e recursos premium.', action: 'Conhecer o Premium' },
]
...
export default function LojaPage() {
  ...
  return <main className="store-page">
    ...
    <section className="plans"><div className="section-title"><span>EXPERIÊNCIA PRETREINO</span><h2>Comece onde fizer sentido para si.</h2></div><div className="plan-grid">{plans.map((plan) => <article key={plan.title} className={plan.highlight ? 'plan highlight' : 'plan'}>{plan.highlight && <div className="popular">MAIS ESCOLHIDO</div>}<small>{plan.tag}</small><h3>{plan.title}</h3><div className="price">{plan.price}<b>{plan.period}</b></div>{plan.annual && <div className="annual">{plan.annual}</div>}<p>{plan.text}</p><button>{plan.action} →</button></article>)}</div></section>

...
export default function IAPage() {
  ...
  async function startConversation() {
    ...
    if (!response.ok || !data.conversation) {
      setMessage(data.detail ? `${data.error || 'Não foi possível iniciar a conversa.'} (${data.detail})` : (data.error || 'Não foi possível iniciar a conversa.'))
      setSending(false)
      return
    }
    const conversation = data.conversation as Conversation
    setItems([conversation, ...items]); setActive(conversation.id); setMessages([])
